...
> **마지막 업데이트:** 2026.2.26 KST
> **마지막 작업:** 어드민 모바일 UI 통일 + CREW앱 안정성 개선 + 로그인 플로우 정리
...
### 다음 작업 후보
1. **CREW앱 실서버 테스트 계속** — 홈 화면 이후 각 기능 검증
2. **미팍티켓 MVP 개발** (고객 화면 + QR + 결제 + 알림톡)
3. **OCR 번호판 인식** — CREW앱 입차등록에 카메라 촬영 추가 (미팍티켓 MVP 후)
4. **출퇴근 GPS 지도** — 카카오맵 JS키 Vercel 등록 + Supabase GPS 컬럼 SQL 실행 필요
...
| 2026.2.27 | CREW앱 출퇴근 GPS 지도 기능: 기획서 업데이트 + GPS좌표 저장 + 카카오맵 컴포넌트 + 매장 거리 표시 + 홈 버튼 | 🔸 DB SQL 실행대기 + 카카오 JS키 등록 필요 |
