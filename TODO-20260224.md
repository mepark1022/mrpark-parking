...
### 다음 작업 우선순위 (2026.3.5 저녁 예정)
1. 🔴 **알림톡 연동 (솔라피)** — 입차확인/차량준비/월주차 D-7 등 5종 템플릿
2. 🔴 **CREW앱 디자인 마무리** — 전체 화면 UI 통일 및 완성도
3. 🟡 **미팍티켓 QR 생성/스캔** — 고객 전자주차권 핵심 기능
4. 🟡 **토스페이먼츠 결제 연동** — PG 결제
5. 🔵 **어드민 대시보드 KPI ↔ mepark_tickets 연동**
...
## ⚠️ 다음 세션 시작 전 Supabase SQL 실행 필요

```sql
-- 1. store_shifts name → shift_name (이미 컬럼 있으면 스킵)
ALTER TABLE store_shifts ADD COLUMN IF NOT EXISTS shift_name TEXT DEFAULT '근무조';

-- 2. store_late_rules 컬럼 추가
ALTER TABLE store_late_rules ADD COLUMN IF NOT EXISTS late_minutes INTEGER DEFAULT 5;
ALTER TABLE store_late_rules ADD COLUMN IF NOT EXISTS absent_minutes INTEGER DEFAULT 30;
ALTER TABLE store_late_rules ADD COLUMN IF NOT EXISTS org_id UUID;

-- 3. stores 기본요금 컬럼 7개 추가
ALTER TABLE stores ADD COLUMN IF NOT EXISTS free_minutes INTEGER DEFAULT 0;
ALTER TABLE stores ADD COLUMN IF NOT EXISTS base_fee INTEGER DEFAULT 1000;
ALTER TABLE stores ADD COLUMN IF NOT EXISTS base_minutes INTEGER DEFAULT 30;
ALTER TABLE stores ADD COLUMN IF NOT EXISTS extra_fee INTEGER DEFAULT 500;
ALTER TABLE stores ADD COLUMN IF NOT EXISTS extra_unit_minutes INTEGER DEFAULT 10;
ALTER TABLE stores ADD COLUMN IF NOT EXISTS daily_max INTEGER DEFAULT 0;
ALTER TABLE stores ADD COLUMN IF NOT EXISTS valet_fee INTEGER DEFAULT 3000;
```
