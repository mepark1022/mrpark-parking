...
| 13.7 | 퇴근 처리 요청 승인 | P1 | ✅ 완료 (2026.2.23) |
| 13.8 | 근태 미출퇴근 집계 | P1 | ✅ 완료 (2026.2.23) |
...
- ⚠️ Supabase SQL 실행 필요 (아래 참조)

**실행할 SQL (Supabase SQL Editor):**
```sql
CREATE TABLE IF NOT EXISTS accident_reports (
  id            uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  org_id        uuid NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  store_id      uuid NOT NULL REFERENCES stores(id) ON DELETE CASCADE,
  accident_at   timestamptz NOT NULL DEFAULT now(),
...
CREATE INDEX IF NOT EXISTS idx_accident_reports_org_id ON accident_reports(org_id);
CREATE INDEX IF NOT EXISTS idx_accident_reports_at ON accident_reports(accident_at DESC);
ALTER TABLE accident_reports ENABLE ROW LEVEL SECURITY;
CREATE POLICY "accident_reports_org_isolation" ON accident_reports
