...
> **다음 작업:** ▶ **P1-2 Part 2 (`/v2/analytics` UI)** — 마지막 P1 잔여. 레거시 `src/app/analytics/page.tsx`(849줄·55KB) 기능을 **API-first 재구축**(Supabase 직접쿼리 제거). 구성: KPI(전기간대비 %, `overview` 사용) + 일별 추이(`daily-trend`, AreaChart) + 매장별 비교(`by-store`) + **시간대별 막대(`/api/v1/stats/hourly`, Part1 신규)** + 결제수단 도넛(`by-payment-method`, 선택). recharts 재사용, 매장/기간 필터, NAVY/GOLD/Outfit, 네임스페이스 `v2an-*`, `credentials:include`. v2 dashboard 차트 패턴 그대로 응용. ⚠️실기기 검증 누적분(P1-4 입차→QR 핸드오프, P1-7 crew 200/타매장403, P1-8 버킷정책, **P1-2 hourly: 긴 기간 truncated 미발생 확인**) 배포 후 1회 점검 권장.
...
> **P1 추천순서(갱신):** ①P1-1✅ → ②~~P1-5~~(보류) → ③**P1-3+P1-6(accidents) ✅** → ④**P1-8 ✅(8a·8b·8c)** → ⑤**P1-7(월주차) ✅(Part1·2·3)** → ⑥**P1-4(QR) ✅** → **⑦P1-2(분석차트) ← 유일 잔여** → ⑧(보류해제 시)P1-5
