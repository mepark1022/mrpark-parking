...
> **작성일:** 2026.04.09
> **마지막 업데이트:** 2026.05.30 (GAP-P1-7 **Part 3(CREW 등록/수정 UI `/v2/crew/monthly/register`) 완료** → **GAP-P1-7 시리즈 100% 종료**)
...
> **마지막 작업:** ✅ **GAP-P1-7 Part 3 (CREW 등록/수정 UI) 완료** — 신규 `src/app/v2/crew/monthly/register/page.tsx`(`@ts-nocheck`, `cv2mreg-*` 네임스페이스, Suspense 래핑). 레거시 `crew/monthly/register/page.tsx`(25KB) 이식하되 **Supabase 직접호출 전면 제거**. 신규=`POST /api/v1/monthly`(store_id 포함, org_id는 API가 세션도출) / `?id=`면 마운트시 `GET /api/v1/monthly/[id]`로 폼 프리필 + 저장시 `PATCH /api/v1/monthly/[id]`(store_id 제외=crew 스코프 불변). 전부 `credentials:include`, 401→`/v2/crew/login` 회귀. 필드: 차량번호(plate 스타일·대문자)·차종·고객명(한글필터)·**연락처(월주차 정책예외=평문수집)**·시작/종료일(MeParkDatePicker, 시작일변경→월말 자동, 1/3/6/12개월 퀵버튼)·월요금·납부상태(미납/완료)·계약상태(수정모드만 노출)·메모. 실시간 미리보기 카드(번호판·기간·계약일수·납부·요금). **409=중복 차량 친절 배너**(`json.error.message`), 403/일반오류도 배너. store_id=localStorage `crew_store_id`(없으면 로그인 회귀). 삭제버튼 미노출(API도 DELETE=MANAGE). BottomNav/NavSpacer는 **v2/crew/layout 상속**(수동 추가 안 함, 저장버튼 인라인). 완료 모달(목록으로/추가등록). ⚠️**기본 월요금 자동조회 보류**: 레거시는 `visit_places` 직접조회였으나 v2 `GET /api/v1/stores/[id]/visit-places`가 **MANAGE 게이트**라 CREW(OPERATE) 호출 불가 → 게이트 추가 완화 대신 기본값 150,000원(편집 가능)으로 처리. 빌드 OK(`✓ Compiled successfully in 76s`, **115p**, `/v2/crew/monthly/register` ○). 경고 2건은 기존 Toss SDK 무관.
> **다음 작업:** 🟢 **GAP-P1-7 시리즈(Part 1·2·3) 100% 완료** — CREW 모바일 월주차 조회·등록·수정 전부 v2 API-first 구현. ▶ 추천 다음 = **P1-4(미팍티켓 QR)** 또는 **P1-2(분석/차트 `/v2/analytics`)**. ▼ 아래 'GAP-P1-7' 블록의 ⚠️실기기 검증(crew 세션 GET/POST/PATCH 200 + 타매장 403) 미완 — 배포 후 1회 점검 권장. P1 잔여 항목은 'GAP 분석 P0/P1/P2' 섹션 참조.
...
> **P1 추천순서(갱신):** ①P1-1✅ → ②~~P1-5~~(보류) → ③**P1-3+P1-6(accidents) ✅ 전체완료** → ④**P1-8 ✅ 전체완료(8a·8b·8c)** → ⑤🟢**P1-7(월주차) ← 진행중(Part1 API✅ / Part2·3 UI 남음)** → ⑥P1-4(QR)·P1-2(차트) → ⑦(보류해제 시)P1-5
