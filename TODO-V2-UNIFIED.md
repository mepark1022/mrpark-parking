...
> **다음 작업:** ▶ **1순위: 실기기 검증 (누적분 한 번에)** — 오늘 코드 변경이 많이 쌓였으니(검증 없이 진행하다 오늘 사고 두 번) 다음 세션은 검증부터 권장. 점검: ①#310 수정확인(/v2/parking-status·crew 근태지도 흰화면 안 뜨는지) ②로그인 랜딩(admin→/v2/dashboard, crew→/crew) ③`/v2/daily-reports` 일보 작성·저장(daily_reports 신규생성분) ④P1-8 `vehicle-photos` 업로드(막히면 sql/storage-vehicle-photos.sql) ⑤P1-7 crew월주차 ⑥P1-4 QR. 체크리스트: `docs/P1-실기기검증-체크리스트.md`. 이후 **3순위(미팍티켓/미톡/홈페이지v4 택1)**. **잔여 정리(별도)**: v1 crew 트리·checkout_requests·orphan v1 admin(/dashboard·/parking-status·/stores·/team)은 crew 컷오버 후. **⚠️대책**: 컴포넌트 수정 시 `npx eslint "src/**/*.tsx" \| grep rules-of-hooks` 선검사 (next build는 lint 미실행).
