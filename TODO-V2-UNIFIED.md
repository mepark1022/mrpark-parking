...
> **작성일:** 2026.04.09
> **마지막 업데이트:** 2026.05.30 (GAP-P1-7 **Part 1(API 권한 완화) 완료** — monthly API를 CREW(OPERATE)에 개방)
...
> **다음 작업:** 🟢 **GAP-P1-7 Part 1(API) 완료.** ▶ 다음 = **Part 2(CREW 조회 UI `/v2/crew/monthly`)** → Part 3(등록/수정 UI `/v2/crew/monthly/register`). ▼ 아래 'GAP-P1-7' 블록 참조. 또는 P1-4(QR)·P1-2(차트)로 전환 가능.
...
**▶ 다음 = Part 2 (CREW 조회 UI)**: `src/app/v2/crew/monthly/page.tsx` 신규. 레거시 `crew/monthly/page.tsx`(18KB) 이식하되 **Supabase 직접조회 제거 → `GET /api/v1/monthly?store_id&search&...`**(credentials:include). store_id=localStorage `crew_store_id`. 검색(차량번호/고객명) + 상태배지(정상/D-N만료임박/만료/해지) + 카드리스트 + 신규등록 버튼(→`/v2/crew/monthly/register`) + 카드클릭 편집(→`register?id=`). 네임스페이스 `cv2mly-*`, NAVY/GOLD/Outfit, CREW BottomNav 상속(v2/crew/layout).
**이후 Part 3 (CREW 등록/수정 UI)**: `src/app/v2/crew/monthly/register/page.tsx` 신규. 레거시 `register/page.tsx`(25KB) 이식 → `POST /api/v1/monthly`(신규) / `?id=`면 `GET`+`PATCH [id]`(수정). 필드: 차량번호·차종·고객명·**연락처(월주차는 평문저장 정책예외=수집함, accidents와 반대)**·시작/종료일(기간개월 자동계산)·월요금(visit_places monthly_fee 자동)·납부상태·계약상태. 409 중복 친절안내. store_id=localStorage.
