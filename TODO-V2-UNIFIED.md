...
> **다음 작업:** 🟢 **P1 잔여 = P1-2(분석 차트)만 남음**(P1-5는 정책 미정 보류/폐기 가능). ▶ **P1-2 `/v2/analytics`** — 레거시 `analytics/page.tsx`(55KB·849줄, 시간대별 차트+매장별 비교+전기간대비 %). 별도 세션급 큰 작업이라 차트 라이브러리(recharts 등 기존 dashboard 확인)·데이터소스(API) 선결점검부터 권장. ▼ 'GAP 분석 P0/P1/P2' 섹션 참조. ⚠️실기기 검증 누적분(P1-4 입차→QR 핸드오프, P1-7 crew 200/타매장403, P1-8 버킷정책) 배포 후 1회 점검 권장.
