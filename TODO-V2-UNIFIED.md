...
### 📄 전자문서함 (근로계약서·사직서·시말서 전자서명) — 기능설계 확정·구현 대기 (2026.06.14 대표)
**정본**: `docs/기획서-전자문서함-전자서명-v1.md`. 어드민 발송 → me.talk 알림 → 크루 설정>전자문서함 **자체 캔버스 서명**(감사메타: SHA-256·타임스탬프·기기/IP·동의체크·감사로그PDF) → 계약 자동연동(임시크루 갱신) → 대시보드 미서명/만기D-7 KPI. 서명 전부 자체캔버스(모두싸인X). **⚠️ 거처 미결** — 인사 마스터(employees/근로계약)가 ⓐ신규 인사 인스턴스(`docs/신규통합앱-계정인사체계-기획서-v1.md` 결정#7) / ⓑparking v2 흡수 / ⓒmrpark-2.0 중 어디냐가 미정이라 **구현 착수 전에 거처(#7) 먼저 확정 필요**. 직원 데이터는 parking v2 `employees`에 이미 존재(이전 부담 작음). 실측: parking v2엔 근로계약 회차 마스터 없음(코드의 contract는 전부 월주차 contract_status), 근로계약 회차 로직은 mrpark-2.0에만.
