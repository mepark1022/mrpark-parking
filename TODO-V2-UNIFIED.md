...
> **작성일:** 2026.04.09
> **마지막 업데이트:** 2026.05.30 (GAP-P1-7 **Part 2(CREW 조회 UI `/v2/crew/monthly`) 완료** — API-first 월주차 조회 페이지 신규)
...
> **마지막 작업:** ✅ **GAP-P1-7 Part 2 (CREW 조회 UI) 완료** — 신규 `src/app/v2/crew/monthly/page.tsx`(`@ts-nocheck`, `cv2mly-*` 네임스페이스). 레거시 `crew/monthly/page.tsx` 이식하되 **Supabase 직접조회 전면 제거 → `GET /api/v1/monthly?store_id&search&contract_status=all&limit=50`**(`credentials:include`, P1-7 Part1에서 게이트 OPERATE 완화로 CREW 호출 가능, API 내부 store 스코핑). store_id=localStorage `crew_store_id`(없으면 `/v2/crew/login` 회귀), 401→로그인 회귀. 검색(차량번호/고객명, Enter+조회버튼), 상태배지 4종(정상/D-N만료임박/만료/해지=`getStatusInfo` 이식), 카드(번호판·고객명·기간·요금·매장·결제배지·진행바·D-N), 신규등록→`/v2/crew/monthly/register`, 카드 수정→`register?id=`(둘 다 Part 3에서 구현). BottomNav/NavSpacer는 **v2/crew/layout 상속**(수동 추가 안 함). NAVY 그라데이션 헤더+골드 등록버튼. 삭제버튼 미노출(API도 DELETE는 MANAGE). 빌드 OK(`✓ Compiled successfully in 81s`, 114p, `/v2/crew/monthly` ○).
> **다음 작업:** 🟢 **GAP-P1-7 Part 1·2 완료.** ▶ 다음 = **Part 3(등록/수정 UI `/v2/crew/monthly/register`)** — 레거시 `register/page.tsx`(25KB) 이식 → `POST /api/v1/monthly`(신규)/`?id=`면 `GET`+`PATCH [id]`(수정). ▼ 아래 'GAP-P1-7' 블록 참조. 또는 P1-4(QR)·P1-2(차트)로 전환 가능.
...
**▶ 다음 = Part 3 (CREW 등록/수정 UI)**: `src/app/v2/crew/monthly/register/page.tsx` 신규. 레거시 `register/page.tsx`(25KB) 이식 → `POST /api/v1/monthly`(신규) / `?id=`면 `GET`+`PATCH [id]`(수정). 필드: 차량번호·차종·고객명·**연락처(월주차는 평문저장 정책예외=수집함, accidents와 반대)**·시작/종료일(기간개월 자동계산)·월요금(visit_places monthly_fee 자동)·납부상태·계약상태. 409 중복 친절안내. store_id=localStorage.
