...
> **다음 작업:** ✅ **2순위(v1→v2 라우팅 일괄교체) 완료 (2026.06.03)** — Sidebar/MobileTabBar/`/more`/MoreSubNav analytics·accident v2화 + Header 타이틀맵 v2 키 11종 추가 + orphan v1 `analytics`/`accident` 페이지 삭제. ▶ **다음 = 1순위(실기기 검증 1회 점검)**: P1-8 `vehicle-photos` 버킷 crew INSERT 정책(최우선 리스크), P1-7 crew 월주차 200/타매장403, P1-4 QR 핸드오프, P1-2 analytics 차트 end-to-end. 검증 체크리스트+막힘 대비 SQL은 다음 세션 준비. **잔여 정리(별도)**: v1 crew 트리 + `checkout_requests`(레거시 stores/delete cascade). 이후 **3순위(미팍티켓/미톡 신규 트랙)**.
