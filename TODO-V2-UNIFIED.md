...
> **작성일:** 2026.04.09
> **마지막 업데이트:** 2026.05.30 (GAP-P1-8 **P1-8b 완료** — v2 입차 차량사진 연속촬영 UI)
...
> **다음 작업:** 🔴 **GAP-P1-8 P1-8c — vehicle-photo cleanup cron**. 신규 `src/app/api/cron/vehicle-photo-cleanup/route.ts` + `vercel.json` crons 등록. `vehicle-photos` 버킷에서 **촬영 2개월 경과 객체만 remove**(ticket row·`vehicle_photos` 컬럼은 유지, Storage 객체만 삭제). 기존 `cron/*` 패턴(demo-cleanup 참고) + CRON_SECRET 검증. ⚠️ 객체 경로 `{org_id}/{ticket_id}/{idx}_{key}.jpg` → prefix 순회 또는 created_at 기준 list. ▼ 상세는 P1-8 블록.
