...
> ⛳ **P3 새 못(v3.3, 대표 승인 2026-07-17): 플레이스토어 전 크루 확산 완료일 = 본 레포(parking) 완전 종료일. 이 레포는 아카이브·소등 대상 — 신규 기능·신규 테이블 절대 금지. 정본 기획서 = mrpark-2.0/docs/미팍통합앱_신규기획서_v3.3.md**
...
> **작성일:** 2026.04.09
> **마지막 업데이트:** 2026.07.02 (토스 PG 심사 대비 방문객 랜딩 + ㉡ 매장명 렌더 수정 + ㉠ 위치 파악)
> **마지막 작업(2026.07.02):** ✅ **소규모 묶음 3건** — ①**ticket.mepark.kr 루트 방문객 랜딩(신규)**: 기존엔 루트 접속 시 middleware가 admin.mepark.kr(앱 선택 런처)로 리다이렉트 → 토스 심사역 URL 직접 접속 시 보완요청 위험. 신규 `src/app/ticket-home/page.tsx`(`@ts-nocheck`, `tkh-*`, 미팍 톤 NAVY/GOLD, 모바일 우선: 히어로+이용 3단계+안내박스+mepark.kr CTA) + `src/middleware.ts` ticket.mepark.kr 분기에 `pathname==="/"` → `rewrite("/ticket-home")` 추가(URL은 `/` 유지, homepage.html 패턴과 동일). admin.mepark.kr 런처는 host 분기 원천 분리라 **무변경**. ②**㉡ 고객 티켓 매장명 미표시 수정**: 원인 = 데이터 아님(공개 API·reshape 모두 store.name 정상) — **렌더에 매장명 요소 자체가 없던 누락**(사용처가 최하단 '문의:' 1곳뿐). `/ticket/[id]/page.tsx` 헤더에 MetalkBadge 아래·상태아이콘 위 매장명 표시 추가(theme.text 색 상속, store null이면 미표시). ③**㉠ 음성 충돌칩 풀번호 검증부 위치 파악(보고만, 수정 없음)**: 본 웹 레포 전체 소스+커밋 이력에 `COLLISION_NEED_FULL`/풀번호 모달/STT 코드 **0건** — 해당 로직은 네이티브 크루앱(`mepark-crew-native`, 로컬 C:\mepark-build\) 소재로 판단. 현 main 서버는 구버전 `confirm_collision=true` 우회 유지(Part 19B-5C, tickets/route.ts 152~154줄) → **서버 가드(409) 미푸시분인지 로컬 점검 필요**. ㉠은 재현 상세 확보 후 클로드 코드(CLI) 네이티브 세션에서 진행. 빌드 OK(`✓ Compiled successfully`, `/ticket-home` ○ 신규, 115p). 기존 tsc 에러 2건(employees/stores·monthly/renew)은 main 기존분으로 무관.
...
> **다음 작업(2026.06.10 갱신2):** ✅ **P1 실기기 검증 STEP2~5 전부 통과(06.10 폰 확인, 이상무)** — 월주차 crew 권한 / QR 핸드오프 / #310 parking-status / 매출분석 헤더 모두 정상. **🎉 P1 시리즈 = 코드+실기기 검증 100% 완전 종료.** ▶ **다음 후보(택1)**: ①신규 통합앱 계정·인사 체계 — 신규 인스턴스 구축 범위 결정(미결 #7: 레포/Supabase 분리 여부, v2 병행 기간) ②crew v2 컷오버(v1 /crew → v2, middleware 기본진입) ③앱선택화면 정리(1257a7b 되돌림 여부) ④미팍티켓·미톡·홈페이지v4. **미해결 설계이슈**: 번호판 4자리만 저장 시 12가2580 vs 34나2580 데이터 충돌(현재 메모수기+사진으로 운영보강).
...
>
> **📌 백로그(예정) — 크루앱 모바일 UI 최적화 (2026.06.14 추가):** 미팍티켓 크루앱(admin.mepark.kr) 모바일 화면 UI 정리·최적화. 스크린샷 기준(`/v2/crew/daily-report/new` 마감 보고): ①폼 밀도·여백·터치타깃 + sticky 액션바(임시저장/제출) 가독성 ②기본정보/입력 카드 스타일 일관성 ③`CrewBottomNav`(홈·현황·출퇴근·마감·설정) 정합·활성표시 ④긴 폼 스크롤 시 액션바 가림/하단 안전영역(safe-area) 처리. 대상=`/v2/crew/*` 위주(마감·출퇴근·입차·월주차·사고). 우선순위 미정(P1 종료 후 후보군과 함께 판단). ⚠️네이티브 전환 예정 → 과한 웹 워크어라운드보다 네이티브에서도 살 구조 우선. ※mrpark-2.0(hr.mepark.kr) 크루와 별개 — 이건 parking 크루앱.
...
### 🧭 방향 고정 (2026.06.15 대표 확정 — 반드시 먼저 읽을 것)
> **parking v2 crew 앱을 더 키우지 말 것.** 통합 CREW앱 정본 = **미팍 2.0**(UI 2.0톤 + 2.0 단일 백엔드, v3.2 §7-1-A). parking에 설정·전자서명 등 신규 구현은 **§3-5 재분열 금지 위반 + 폐기될 코드**.
> - **parking이 통합 CREW에 넘기는 건 입출차·월주차·사고뿐** → 2.0로 이관 대상. 출퇴근·마감·전자서명·설정·홈은 **2.0 정본**.
> - **❌ 보류(미룸) 확정:** parking `/v2/crew/settings` 신규 구현, crew 컷오버(parking v2를 라이브 기본으로). — 이유: 정본이 2.0이라 parking v2 굳히면 2.0으로 또 이사.
> **▶ 안전한 단계(2.0 기준):** 1) P0 로그인 단일화(파트2✅·파트3 동결 남음) → 2) 통합 CREW 셸(2.0, crew 홈·하단네비, 2.0톤) → 3) 입출차·월주차·사고 이관(parking→2.0, 요금 facet 과도기 잔류) → 4) 출퇴근·마감(2.0 정본, 기존✅) → 5) **전자서명 크루측(2.0 파트5)**: 통합 CREW "설정>전자문서함" 수신·캔버스 서명·급여명세 확인 → 6) 컷오버 → 7) 네이티브=DB 통합 디데이.
> **전자서명 현황(2.0):** 발송·템플릿·AI초안 코드완료(라이브 검증 대기), **크루 수신·서명(파트5) 미구현**. 통합 CREW 셸(단계2)이 서야 파트5가 올라감.
> **세션 교훈(2026.06.15):** parking에 v1·v2 crew 로그인 2벌 존재(`/crew/login`=Supabase직접 / `/v2/crew/login`=API). 라이브 기본진입은 아직 v1. v2 crew는 13페이지 구현됐으나 **설정 페이지 없음(탭만 있고 404)**. v2 톤은 매장 배정돼야 홈 진입으로 확인됨(매장 미배정 시 빈화면이 v1처럼 보임). 이 모든 게 "2.0로 수렴" 방향이라 parking에서 메우지 말 것.
...
| 날짜 | Part | 작업 내용 | 결과 | 커밋 |
|------|------|----------|------|------|
| 2026.06.10 | P1 실기기 검증 STEP2~5 | 잔여 미검증 4건 폰 검증 완료(이상무). **STEP2(P1-7)**: crew 세션 월주차 조회·신규등록·수정 정상, 중복 409 친절배너 정상, 삭제버튼 미노출 확인. **STEP3(P1-4)**: 입차 성공→QR 화면 자동이동(토스트X)·풀스크린 네이비·하단탭 숨김·링크공유·QR스캔→`/ticket/[id]` 고객뷰 정상. **STEP4(#310)**: `/v2/parking-status` 흰화면 재발 없음(06.03 hooks 수정분 검증 완료). **STEP5(P1-2)**: `/v2/analytics` 헤더 '매출 분석' 정상(타이틀맵 버그 수정분 검증), KPI·시간대별 peak 골드·92일 안내·매장필터 자동재로드 정상. 체크리스트 문서에 검증 결과 기록 후 아카이브 처리 | ✅ **P1 시리즈 코드+실기기 검증 100% 완전 종료** (코드 변경 없음, 문서 2건만) | (이번 push) |
| 2026.06.10 | 신규 통합앱 계정·인사 체계 기획 (대표 단답 세션) | **`docs/신규통합앱-계정인사체계-기획서-v1.md` 신규 작성** — 대표 메모→단답 확정 7건 총합. **⚠️핵심: 기존 v2 인증체계(사번@internal+뒤4자리+12)와 전혀 다른 완전 신규 체계, 신규 인스턴스에서 신축(기존 v2 무수정)**. 확정: ①역할 4단계 owner/super_admin/admin/crew ②크루=단일역할 통칭, 주00일=직원현황 수기필드 ③**개인마감=fact 정본**(사전지정 폼→사후마감 패러다임 전환)→AI 1차검증→이상건만 어드민 승인→근태·급여 반영 ④field_member 폐지 ⑤크루 구분=기간 유무(임시크루=어드민 기간설정+AI 갱신제안, store_members.valid_until) ⑥**ID=연락처11자리/PW=숫자6자리**(초기 123456, 첫 로그인 변경 강제) ⑦owner 시드=010-9535-7333/442812. +출퇴근 AI(평상시와 다른 시간·업장 확인), 마감 이원화(개인마감 전원/매출마감 지정인원), 본사 대시보드(해당월 기본+전달/앞달/분기/연간/특정기간), 직원별 AI리뷰, hr_system_(13).html 기능 이식 참고(reportBase·계약타임라인·은행업로드 — 민감정보 평문 주의). DB 스키마 초안+후속과제 8건 문서에 포함 | ✅ 기획서 v1.0 작성 완료 (코드 변경 없음, 문서만) | (이번 push) |
